...
int make_move(short int old_pos, short int coin, short int Ns, const std::string& graph){
    int new_pos; //Declare new position variable
    int number_choices;//Declare variable that will be used to decide array length

    const std::string& cycle = "cyclic"; //String constant

   ;//Returns zero if the two character arrays are identical
    
    if (graph == cycle)//If we are on a cycle graph 
        {
...
int quantum_table_index(short int Ns, short int player1position, short int player2position, const std::string& graph, bool check_first) {
    int counter = 0; // counter for array
    int final_counter = -1; // initialize final_counter

    const std::string& cycle = "cyclic"; // String array for graph names
    

    if (graph ==cycle) { // If we have a cycle graph
        for (int i = 0; i < Ns; i++) { // iterate through first player sites
            for (int j = 0; j < Ns; j++) { // iterate through second player sites
                if (i == j && check_first == true) { // if possible positions match check to see if it's allowed
                    //std:: cout << "I am here 1 "; Debug statement
                   ...
                   // std:: cout << "I am here 2 "; Debug Statement
                }
                else{
                    counter++;
                   // std:: cout << "I am here 3 "; Debug statement
                }
            }  
        }
    }
...
bool player_move(int initial_pos[], const std::string& graph, int Np, bool edge, int quantum_table_pointer,bool check_first, int Ns, int Nm, const std::string& strategy){
    ...
    while(win==false && j <Nm){
        ...
        else if (strategy == "classical_go_to_lowest")
        {
            //std:: cout<<"yes";
            coin =1;//placeholder 
        }
...
float many_runs(const std::string& graph, int Np, bool edge, int quantum_table_pointer,bool check_first, int Ns, int Nr, int Nm, const std::string& strategy){
...
float run_game(){
    ...
    const std::string& graph = "cyclic";//What graph are we playing on 
    const std::string& strategy = "classical_go_to_lowest";//What Strategy are the players using
  ...
  //  int *quantum_table_pointer;//define integer pointer 
   int quantum_table_pointer = 0 ;//Set the pointer to the table

    clock_t start = clock();//Start clock
    number_wins = many_runs(graph,2,edge,quantum_table_pointer,true,Ns,Nr,Nm,strategy);//run many_runs function
    //std::cout << "Number of wins" << number_wins;//Prints number of wins
   ...
   // std::cout << win_percent;//Print win percentage
    return win_percent;
}
